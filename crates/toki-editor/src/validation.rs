...
impl AssetValidator {
    ...
    fn validate_and_log_file(&self, file_path: &Path, schema_type: &str, asset_name: &str) -> bool {
        ...
        let content = match fs::read_to_string(file_path) {
            Ok(content) => content,
            Err(e) => {
                tracing::error!("❌ {} '{}': Failed to read file {}: {}", schema_type, asset_name, file_path_str, e);
                return false;
            }
        };
        ...
        let json_value: Value = match serde_json::from_str(&content) {
            Ok(value) => value,
            Err(e) => {
                tracing::error!("❌ {} '{}': Invalid JSON in {}: {}", schema_type, asset_name, file_path_str, e);
                return false;
            }
        };
        ...
        let schema = match self.schemas.get(schema_type) {
            Some(schema) => schema,
            None => {
                tracing::error!("❌ {} '{}': No schema found for type: {}", schema_type, asset_name, schema_type);
                return false;
            }
        };
        ...
        match validation_result {
            ...
            Err(errors) => {
                tracing::error!("❌ {} '{}': Schema validation failed:", schema_type, asset_name);
                for error in errors {

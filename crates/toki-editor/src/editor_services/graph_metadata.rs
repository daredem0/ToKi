...
pub fn load_into_ui(ui_state: &mut EditorUI, project: Option<&Project>) {
    let (graph_layouts, rule_graph_drafts, dialog_graph_layouts, ui_layout_views) = project
        .map(|project| {
            (
                ...
                project.metadata.editor.ui_layout_views.clone(),
            )
        })
        .unwrap_or_default();
    ...
    crate::ui::editor_ui::load_ui_layout_views_from_project(ui_state, &ui_layout_views);
}
...
pub fn persist_if_dirty(
    ...
    if !crate::ui::editor_ui::is_graph_layout_dirty(ui_state)
        ...
        && !crate::ui::editor_ui::is_ui_layout_view_dirty(ui_state)
    {
        return;
    }
    ...
    match project.save_metadata() {
        Ok(()) => {
            ...
            crate::ui::editor_ui::clear_ui_layout_view_dirty(ui_state);
        }
...
pub fn copy_ui_into_project(ui_state: &EditorUI, project: &mut Project) {
    ...
    project.metadata.editor.ui_layout_views =
        crate::ui::editor_ui::export_ui_layout_views_for_project(ui_state);
}

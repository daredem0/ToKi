...
impl EditorUI {
    pub fn load_graph_layouts_from_project(
        ...
        crate::ui::editor_context::graph_state_mut(self).layouts_by_scene = graph_layouts.clone();
        crate::ui::editor_context::graph_state_mut(self).layout_dirty = false;
    }

    pub fn load_rule_graph_drafts_from_project(&mut self, drafts: &HashMap<String, RuleGraph>) {
        crate::ui::editor_context::graph_state_mut(self).rule_graphs_by_scene = drafts.clone();
    }

    pub fn export_graph_layouts_for_project(&self) -> HashMap<String, SceneGraphLayout> {
        crate::ui::editor_context::graph_state(self).layouts_by_scene.clone()
    }

    pub fn export_rule_graph_drafts_for_project(&self) -> HashMap<String, RuleGraph> {
        crate::ui::editor_context::graph_state(self).rule_graphs_by_scene.clone()
    }

    pub fn is_graph_layout_dirty(&self) -> bool {
        crate::ui::editor_context::graph_state(self).layout_dirty
    }
    ...
    pub fn graph_layout_position(&self, scene_name: &str, node_key: &str) -> Option<[f32; 2]> {
        crate::ui::editor_context::graph_state(self)
            .layouts_by_scene
            .get(scene_name)
    ...
    pub fn graph_view_for_scene(&self, scene_name: &str) -> (f32, [f32; 2]) {
        if let Some(layout) = crate::ui::editor_context::graph_state(self).layouts_by_scene.get(scene_name) {
    ...
    pub fn set_graph_view_for_scene(&mut self, scene_name: &str, zoom: f32, pan: [f32; 2]) {
        let layout = crate::ui::editor_context::graph_state_mut(self)
            .layouts_by_scene
        ...
        if (layout.zoom - zoom).abs() > f32::EPSILON || layout.pan != pan {
            layout.zoom = zoom;
            layout.pan = pan;
            crate::ui::editor_context::graph_state_mut(self).layout_dirty = true;
        }
    }
    ...
    pub fn sync_rule_graph_with_rule_set(&mut self, scene_name: &str, rule_set: &RuleSet) {
        let needs_rebuild = match crate::ui::editor_context::graph_state_mut(self).rule_graphs_by_scene.get(scene_name) {
            None => true,
    ...
    pub fn rule_graph_for_scene(&self, scene_name: &str) -> Option<&RuleGraph> {
        crate::ui::editor_context::graph_state(self).rule_graphs_by_scene.get(scene_name)
    }

    pub fn set_rule_graph_for_scene(&mut self, scene_name: String, graph: RuleGraph) {
        crate::ui::editor_context::graph_state_mut(self).rule_graphs_by_scene.insert(scene_name, graph);
    }
}

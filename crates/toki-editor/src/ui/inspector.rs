...
impl InspectorSystem {
            ...
            .show(ctx, |ui| {
                ...
                egui::ScrollArea::vertical()
                    ...
                    Some(Selection::Map(scene_name, map_name)) => {
                        ...
                        Self::render_map_details(ui, map_name, config, Some(scene_name), &mut ui_state.map_load_requested);
                    },
                    ...
                    Some(Selection::StandaloneMap(map_name)) => {
                        ...
                        Self::render_map_details(ui, map_name, config, None, &mut ui_state.map_load_requested);
                    },
                                ...
                                Ok(json) => {
                                                ...
                                                for (key, value) in obj {
                                                    ui.horizontal(|ui| {
                                                        ...
                                                        let value_str = match value {
                                                            ...
                                                            serde_json::Value::Object(obj) => format!("{{{}}} properties", obj.keys().count()),
                                    ...
                                    if let Some(scene_name) = scene_name {
                                        if ui.button("📂 Load in Viewport").clicked() {
                                            tracing::info!("Load Map '{}' from scene '{}' clicked", map_name, scene_name);
                                            *map_load_requested = Some((scene_name.to_string(), map_name.to_string()));
                                        }
                                    } else {
    ...
    pub fn render_entity_definition_details(ui: &mut egui::Ui, entity_name: &str, config: Option<&EditorConfig>) {
                                ...
                                Ok(json) => {
                                    ...
                                    if let Some(obj) = json.as_object() {
                                        ...
                                        if let Some(rendering) = obj.get("rendering").and_then(|v| v.as_object()) {
                                            ...
                                            if let Some(visible) = rendering.get("visible").and_then(|v| v.as_bool()) {
                                                ui.horizontal(|ui| {
                                                    ui.label("Visible:");
                                            ...
                                            if let Some(render_layer) = rendering.get("render_layer").and_then(|v| v.as_u64()) {
                                                ui.horizontal(|ui| {
                                        ...
                                        if let Some(attributes) = obj.get("attributes").and_then(|v| v.as_object()) {
                                            ...
                                            if let Some(health) = attributes.get("health").and_then(|v| v.as_u64()) {
                                                ui.horizontal(|ui| {
                                                    ui.label("Health:");
                                                    ui.label(format!("{}", health));
                                                });
                                        ...
                                        if let Some(audio) = obj.get("audio").and_then(|v| v.as_object()) {
                                            ...
                                            if let Some(distance) = audio.get("footstep_trigger_distance").and_then(|v| v.as_f64()) {
                                                ui.horizontal(|ui| {
                                                ...
                                                ui.indent("animation_clips", |ui| {
                                                    for clip in clips.iter() {
                                                        if let Some(clip_obj) = clip.as_object() {
                                                            ...
                                                            let frame_duration = clip_obj.get("frame_duration_ms").and_then(|v| v.as_f64()).unwrap_or(0.0);
                                                            let frame_count = clip_obj.get("frame_tiles").and_then(|v| v.as_array()).map(|arr| arr.len()).unwrap_or(0);
                                                            
                                                            ui.horizontal(|ui| {
                                                                ui.label(format!("• {}: {} frames, {:.0}ms, {}", state, frame_count, frame_duration, loop_mode));
                                                            });
                                                        }
                                                    }
                                                });

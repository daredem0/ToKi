...
pub fn render_entity_editor(
    ...
    if crate::ui::editor_context::entity_editor_state_mut(ui_state).entities.is_empty() && project_path.is_some() {
        io::refresh_entity_list(ui_state, project_path.as_deref());
    }

    // Handle dialogs
    if crate::ui::editor_context::entity_editor_state_mut(ui_state).new_entity_dialog.is_open {
        dialogs::render_new_entity_dialog(ui_state, ctx, project_path.as_deref());
    }
    if crate::ui::editor_context::entity_editor_state_mut(ui_state).delete_confirmation.is_open {
...
fn render_main_content(
    ...
    let browser_width = crate::ui::editor_context::entity_editor_state_mut(ui_state).browser_panel_width;
    ...
    ui.horizontal(|ui| {
        ...
        if sep_response.dragged() {
            crate::ui::editor_context::entity_editor_state_mut(ui_state).browser_panel_width =
                (crate::ui::editor_context::entity_editor_state_mut(ui_state).browser_panel_width + sep_response.drag_delta().x)

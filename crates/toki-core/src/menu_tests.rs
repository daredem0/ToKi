...
#[test]
fn menu_controller_returns_save_and_load_commands_for_slot_actions() {
    let settings = MenuSettings {
        pause_root_screen_id: "pause_menu".to_string(),
        gate_gameplay_when_open: true,
        appearance: Default::default(),
        screens: vec![MenuScreenDefinition {
            ...
            items: vec![
                MenuItemDefinition::Button {
                    text: "Save".to_string(),
                    border_style_override: None,
                    action: UiAction::SaveGame { slot: 2 },
                },
                MenuItemDefinition::Button {
                    text: "Load".to_string(),
                    border_style_override: None,
                    action: UiAction::LoadGame { slot: 3 },
                },
            ],
        }],
        dialogs: Vec::new(),
    ...
    assert_eq!(
        controller.handle_input(MenuInput::Confirm),
        Some(UiCommand::SaveGame { slot: 2 })
    );
    controller.handle_input(MenuInput::Down);
    assert_eq!(
        controller.handle_input(MenuInput::Confirm),
        Some(UiCommand::LoadGame { slot: 3 })
    );
}

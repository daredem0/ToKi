...
impl ResourceManager {
    ...
    pub fn load_from_project_dir(project_dir: &Path) -> Result<Self, ResourceError> {
        ...
        let tilemap = TileMap::load_from_file(assets_dir.join("maps").join("new_town_map_64x64_crossings.json"))
    ...
    pub fn load_with_paths(
        ...
        let terrain_atlas = AtlasMeta::load_from_file(terrain_atlas_path)
            .map_err(|_| ResourceError::Validation(format!("Failed to load terrain atlas: {}", terrain_atlas_path.display())))?;
            
        let creature_atlas = AtlasMeta::load_from_file(creature_atlas_path)
            .map_err(|_| ResourceError::Validation(format!("Failed to load creature atlas: {}", creature_atlas_path.display())))?;
            
        let tilemap = TileMap::load_from_file(tilemap_path)
            .map_err(|_| ResourceError::Validation(format!("Failed to load tilemap: {}", tilemap_path.display())))?;

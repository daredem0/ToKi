...
fn event_queue_fifo_ordering() {
    ...
    assert_eq!(queue.pop(), Some(TestEvent::EventWithData("first".to_string())));
    assert_eq!(queue.pop(), Some(TestEvent::EventWithData("second".to_string())));
    assert_eq!(queue.pop(), Some(TestEvent::EventWithData("third".to_string())));
...
fn game_update_result_combined_movement_and_events() {
    ...
    assert_eq!(result.events[1], TestEvent::EventWithData("moved".to_string()));
}

...
fn create_collision_test_tilemap() -> TileMap {
    let tiles = vec![
        "floor".to_string(), "stone".to_string(), "floor".to_string(), "floor".to_string(),
        "floor".to_string(), "stone".to_string(), "stone".to_string(), "floor".to_string(),
        "floor".to_string(), "floor".to_string(), "floor".to_string(), "floor".to_string(),
        "floor".to_string(), "floor".to_string(), "floor".to_string(), "floor".to_string(),
    ];
...
fn collision_entity_larger_than_tiles() {
    ...
    if let Some(player) = game_state.entity_manager_mut().get_entity_mut(player_id) {
        player.position = IVec2::new(0, 0);
        player.collision_box = Some(CollisionBox::solid_box(UVec2::new(24, 24))); // Spans 3x3 tiles
    }

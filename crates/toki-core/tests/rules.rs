...
fn dialog_completion_trigger_can_drive_follow_up_rules() {
    ...
    let _player = player_entity(&state).expect("player should exist");
    assert_eq!(
        player_inventory(&state)
...
fn on_tile_exit_fires_when_entity_leaves_tile() {
    ...
    assert!(
        exited,
        "OnTileExit should fire when entity leaves the specified tile"
    );
}
...
fn remove_inventory_item_never_goes_negative() {
    ...
    assert_eq!(
        ...
        0,
        "Should remove all but not go negative"
    );
}
